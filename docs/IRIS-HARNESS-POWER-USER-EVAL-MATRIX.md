...
| PU-08 | **Typed question with Codex but no screen help:** “What is a USB-C cable?” followed by a screen-dependent request such as “Which Kneecap folder is open?” | A usable Codex login enables only a non-empty, unsized typed request. The route sends no screenshot, local files, terminal output, or claimed current-app facts; screen-dependent requests remain explicitly unavailable until screen help connects. Existing coverage: `CodexTextOnlyAskChecks.swift` plus `ComposerConnectionPresentationTests`. | In the freshly built Iris Test app, enter a safe typed question and observe Send enabled with “General questions through Codex.” Then verify the visible copy says to connect screen help for the current Mac or screen. Do not send a provider request merely to test button state. | Native acceptance is pending: an old `/Applications/Iris Test.app` window still showed the previous disabled-send state, while the exact current Test process did not expose a reachable window through accessibility. Deterministic host evidence passes; live provider behavior remains unverified. |
