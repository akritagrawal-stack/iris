...
4. Expose a typed workspace binding with guide revision and original/staged identity. Do not claim a bound guide if hard-coded commands still point home. Document the minimal schema change the integrator must wire.
...
Owner: Luna High, Terra reviews the storage contract before mutations. Scope first: `PendingEditCandidateIdentity.swift`, `MaintainSavedChangeRechecker.swift`, associated durable existing candidate state and tests. Scope second: `AppDeliveryReceiptStore.swift`, `SavedUndoRecoverySection.swift` and `BackupRetentionChecks.swift`. One owner at a time; no shared storage edits.
