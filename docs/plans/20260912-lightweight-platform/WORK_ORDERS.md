...
Source review already confirms bounded reverse consumers are wired and source-like `return false`/`throw new Error` compaction is implemented and tested. Preserve those fixes; do not reimplement them. Record the static relative-import/100 mapped-file limit honestly. Start with outcome attribution and offline replay of an actual remaining complex failure, inspecting requirement evidence, current call-site context and independent reviewer findings. A new selector change requires a demonstrated failing case. Preserve instruction/failure summaries within current bytes, review reserve, cancellation and sandbox limits. No broad repository embedding index.
