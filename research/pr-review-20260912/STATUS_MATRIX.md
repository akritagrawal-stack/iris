...
| Earlier PlantGPT search/lifecycle | Historical actual UI | Narrow update/relaunch/restart Undo preserved project data. The accepted `0db25ba…` source/artifact still exists, but the registered checkout is at base and no replayable accepted-edit coordinator state is available; fresh full delivery/restart/Undo remains unverified. |
